...
def check_naive_weighted(iterations: int):
    size = 5
    largest_region = 7
    max_regions = 25
    removals = []
    ...
    for i in range(0,iterations):
        gen.generate_fillomino()
        size = len(gen.generate_puzzle_naive_weighted())
        removals.append(size)
    plt.hist(removals, bins=range(min(removals), max(removals) + 2), align='left', rwidth=0.8, color='skyblue', edgecolor='black')
    max_count = removals.count(most_frequent(removals)) 
...
def check_top_down_weighted(iterations:int):
    size = 5
    largest_region = 7
    max_regions = 25
    removals = []
    ...
    for i in range(0,iterations):
        gen.generate_fillomino()
        size = len(gen.generate_puzzle_weighted())
        removals.append(size)
    plt.hist(removals, bins=range(min(removals), max(removals) + 2), align='left', rwidth=0.8, color='skyblue', edgecolor='black')
    max_count = removals.count(most_frequent(removals)) 
